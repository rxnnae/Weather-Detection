...
if (sensorValue2<4)	// If rain sensor value > threshold
{
Serial.println("rain is detected ");	// Print on serial monitor that rain detected
	Serial.println(sensorValue2);
digitalWrite(enable2, HIGH);	// Light the LED on board flag = true;
// Rain status is true 7
 
}
else	// If rain sensor value < threshold
{
Serial.println("rain not detected ");	// Print on serial monitor that rain detected
digitalWrite(enable2, LOW);		// Switch off the LED on board flag = false;	// Rain status is false
}
...
ThingSpeak.writeField(myChannelNumber, 3, sensorValue2, myWriteAPIKey);	// Send sensorValue of rain to ThingSpeak in channel 3
//delay(100);
